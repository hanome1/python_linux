...
class TestPositive:
    ...
    def test_step2(self):
        ...
        result1 = checkout(f'cd {data["folder_out"]}; 7z e arx2.7z -o/{data["folder_ext"]} -y',
                           "Everything is Ok")
    ...
    def test_step3(self):
        ...
        assert checkout(f"cd {data['folder_out']}; 7z t arx2.7z", "Everything is Ok"), "Test3 FAIL"

    def test_step4(self):
        # test4 ========= add archive update
        assert checkout(f"cd {data['folder_in']}; 7z u {data['folder_out']}arx2.7z", "Everything is Ok"), "Test4 FAIL"

    def test_step5(self):
        # test5 ========= delete docs one and two from archive in folder out
        assert checkout(f"cd {data['folder_out']}; 7z d arx2.7z", "Everything is Ok"), "Test5 FAIL"
        pass

    def test_step6(self, clear_folders, make_folders, make_files):
        """TESTING COMMAND l"""
        subprocess.run(f'cd {data["folder_in"]}; 7z a {data["folder_out"]}arx2', shell=True, stdout=subprocess.PIPE, encoding='utf-8')
...
class TestNegative:
    def test_step1(self):
        ...
        assert checkout_negative("cd {}; 7z e bad_arx.7z -o/{} -y".format({data["folder_out"]}, {data["folder_ext"]}), 'ERRORS')
        assert checkout_negative(f'cd {data["folder_out"]}; 7z e bad_arx.7z -o{data["folder_ext"]} -y', 'ERRORS')
    ...
    def test_step2(self):
        ...
        assert checkout_negative(f'cd {data["folder_out"]}; 7z t bad_arx.7z', "ERRORS"), 'Test3 FAIL'
